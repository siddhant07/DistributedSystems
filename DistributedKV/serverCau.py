import zmq
import threading
from config import HOST, causal_port, kv_store_name, causal_dir, replica_cnt, broadcast_port
...
class Server:
    ...
    def handle_client(self, client_socket, lock, broadcastPort, primary_comm):
        while True:
            ...
            parts = command.split(" ")
            cmd = parts[0].lower()
            storage_path = os.path.join(causal_dir, self.kv_store)
        ...
        while True:
            for i in range(replica_cnt):
                if causal_port+i != self.port:
                    ...
                    storage_path = os.path.join(causal_dir, fileName)
...
if __name__=="__main__":
    ...
    files = os.listdir(causal_dir)       #cleans the output folder whenever the server is restarted

    for file in files:
        filepath = os.path.join(causal_dir, file)
        os.remove(filepath)

    for i in range(replica_cnt):
        kv_file = f"{kv_store_name}{causal_port+i}.txt"
        server = Server(HOST, causal_port+i, kv_file, broadcast_port+i)
