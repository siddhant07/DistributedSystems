import zmq
import os
from config import HOST, linearizable_port, kv_store_name, linearizable_dir, replica_cnt, linearizable_prim_port, broadcast_port
...
class Server:
    def __init__(self, host, port, kv_store, linearizable_host):
        self.host = host
        self.port = port
        self.kv_store = kv_store
        self.linearizable_host = linearizable_host
        self.id = 1
        self.que = []
    ...
    def update_store(self, command):
        ...
        storage_path = os.path.join(linearizable_dir, filename)
        resp = ""
    ...
    def handle_replica(self, primary_comm):
        while True:
            ...
            ack_sock.connect(f"tcp://{HOST}:{self.linearizable_host}")
    ...
    def start(self):
        ...
        primary_socket.connect(f"tcp://{HOST}:{self.linearizable_host}")
        ...
        client_thread = threading.Thread(target=self.handle_client, args=(client_socket, primary_socket, lock, self.linearizable_host))
        client_thread.start()
...
if __name__=="__main__":
    ...
    files = os.listdir(linearizable_dir)       #cleans the output folder whenever the server is restarted

    for file in files:
        filepath = os.path.join(linearizable_dir, file)
        os.remove(filepath)

    for i in range(1, replica_cnt+1):
        kv_file = f"{kv_store_name}{linearizable_port+i}.txt"
        server = Server(HOST, linearizable_port+i, kv_file, linearizable_prim_port+i)
